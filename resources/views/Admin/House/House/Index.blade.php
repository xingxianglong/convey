        ...
        <div class="layui-col-xs11">
            <div class="grid-demo grid-demo-bg1">
                ...
                <div class="layui-inline select-input">
                    ...
                    <div class="layui-input-inline">
                        <input type="text" name="house_code" autocomplete="off" placeholder="请输入编号" class="layui-input">
                    </div>
                </div>
            </div>
        </div>
...
layui.use(['table','laydate','form'] , function(){
    ...
    table.render({
        ...
        ,cols: [[ //标题栏
            ...
            ,{field: 'house_code', title: '编号',width:100}
            ,{field: 'title', title: '标题',minWidth:100}
            ,{field: '', title: '门店',width:180,templet:function(d){
                var str = d.store_name;
                if(d.store_note)
                {
                    str += '(' + d.store_note + ')'
                }
            ...
            ,{field: 'consultant_name', title: '顾问',width:90}
            ,{field: 'community_name', title: '小区',width:120}
            ,{field: 'classify_name', title: '分类',width:90}
            ,{field: '', title: '封面图',width:130,templet:function(d){
                var str = '<img src="'+ d.cover +'" style="" />';
                return str;
            }}
            ,{field: 'direction_name', title: '朝向',width:70}
            ,{field: 'decorate_name', title: '装修',width:100}
            ,{field: 'way_name', title: '付款方式',width:90}
            ,{field: 'apartment_name', title: '品牌公寓',width:90}
            ,{field: '', title: '室厅卫阳台厨',width:160,templet:function(d){
                var str = d.room + '室' + d.hall + '厅' + d.toilet + '卫' + d.balcony + '阳台' + d.kitchen + '厨';
                return str;
            }}
            ,{field: 'rent', title: '租金',width:90}
            ,{field: 'acreage', title: '面积',width:80}
            ,{field: '', title: '楼层',width:80,templet:function(d){
                var str = d.floor + ' / ' + d.total_floor;
                return str;
            }}
            ,{field: '', title: '优选好房',width:90,templet:function(d){
                    var str = '否';
                    if(d.is_optimization == 1)
                    {
                        str = '是';
                    }
                    return str;
                }}
            ,{field: '', title: '整租合租',width:90,templet:function(d){
                    var str = '整租';
                    if(d.entire_or_joint == 2)
                    {
                        str = '合租';
                    }
                    return str;
                }}
            ,{field: '', title: '钥匙',width:60,templet:function(d){
                var str = '没有';
                if(d.is_key == 1)
                {
                    str = '有';
            ...
            ,{field: '', title: '电梯',width:60,templet:function(d){
                var str = '没有';
                if(d.is_elevator == 1)
                {
                    str = '有';
        ...
        ,done:function(res, curr, count) {
            $('.laytable-cell-1-0-8').click('on', function () {
                var that = $(this);
