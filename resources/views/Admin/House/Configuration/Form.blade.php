        ...
        <div class="layui-col-md6">
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <input type="text" name="configuration_name" value="{{isset($info->configuration_name)?$info->configuration_name:''}}" autocomplete="off" placeholder="请输入配置名称" class="layui-input">
                </div>
            </div>
                ...
                <div class="layui-input-block">
                    <div class="upload-img">
                        <div class="sample-img">
                            <img src="{{isset($info->icon) ? $info->icon : ''}}" alt=""/>
                            <input type="hidden" name="icon" value="{{isset($info->icon) ? $info->icon : ''}}" />
                            <input type="hidden" name="icon_ext" value="{{isset($info->icon_ext) ? $info->icon_ext : ''}}" />
                            <input type="hidden" name="icon_size" value="{{isset($info->icon_size) ? $info->icon_size : ''}}" />
                        </div>
                        <button type="button" class="layui-btn" id="icon-img">
...
layui.use(['form', 'layedit', 'laydate','upload','element','table'], function(){
    ...
    form.on('submit(demo1)', function(data){
        ...
        if(jump_type == 1)
        {
            url = '{{"/Admin/House/Configuration/Add"}}';
        }
        else if(jump_type == 2)
        {
            url = '{{"/Admin/House/Configuration/Edit"}}';
        }
        else
        {
        ...
        $.ajax({
            ...
            data:{
                id:id,
                configuration_name:data.field.configuration_name,
                icon:data.field.icon,
                icon_ext:data.field.icon_ext,
