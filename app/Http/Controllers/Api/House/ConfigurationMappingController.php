...
class ConfigurationMappingController extends BaseController
{
     ...
     * @return array
     */
    public function AccordingHouseGetData(Request $request,$house_id)
    {
        //配置
        $configuration = new ConfigurationController($request);
        $configuration_res = $configuration->GetList($request);
        $data = $configuration_res['data'];
        foreach($data as $k => $v)
        {
            $is_exist = 0;
            $where = array(
                ...
                [$this->table_name.'.configuration_id','=',$v->id],
            );
     ...
     */
    public function AccordingHouseUpdateConfiguration($house_id,$configuration_id,$user_id)
    {
        // 启动事务
        Db::beginTransaction();
        try {
