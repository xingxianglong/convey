...
class CityController extends BaseController
{
     ...
     */
    public function AccordingProvinceIdNameGetInfo($province_id,$city_name)
    {
        //默认条件
        $where = array(
            [$this->table_name.'.province_id','=',$province_id],
            [$this->table_name.'.city_name','=',$city_name],
        );

        $info = DB::table($this->table_name)
            ->select($this->table_name.'.'.$this->table_pk.' as id',$this->table_name.'.province_id',$this->table_name.'.city_name',$this->table_name.'.is_delete')
            ->where($where)
            ->first();
        if(empty($info))
        {
