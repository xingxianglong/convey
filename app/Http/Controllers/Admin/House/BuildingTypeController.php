...
use App\Rules\Admin\House\BuildingType as RulesBuildingType;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class BuildingTypeController extends BaseController
{
    public $table_name = 'web_house_building_type';
    public $table_pk = 'type_id';
    ...
    {
        return view('Admin/House/BuildingType/Index');
    }
    ...
    public function GetPage(Request $request)
    {
        ...
        if(count($param) > 0){
            ...
            if(isset($param['type_name']) && !empty($param['type_name'])){
                $where[] = [$this->table_name.'.type_name','like','%'.trim($param['type_name']).'%'];
            }
    ...
    public function Form($jump_type,$id=0)
    {
        ...
        return view('/Admin/House/BuildingType/Form',$data);
    }
     ...
     * @param $validator
     */
    public function Add(Request $request,RulesBuildingType $validator)
    {
        ...
        if(!empty($info))
        {
            return $this->common_class->ajaxDataReturnFormat(1,'建筑类型已存在，请勿重复添加');
        }
     ...
     * @param $validator
     */
    public function Edit(Request $request,RulesBuildingType $validator)
    {
     ...
     * @param $validator
     */
    public function Delete(Request $request,RulesBuildingType $validator)
    {
