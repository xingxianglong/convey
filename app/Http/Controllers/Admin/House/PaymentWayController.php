...
use App\Rules\Admin\House\PaymentWay as RulesPaymentWay;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class PaymentWayController extends BaseController
{
    public $table_name = 'web_house_payment_way';
    public $table_pk = 'way_id';
    ...
    {
        return view('Admin/House/PaymentWay/Index');
    }
    ...
    public function GetPage(Request $request)
    {
        ...
        if(count($param) > 0){
            ...
            if(isset($param['way_name']) && !empty($param['way_name'])){
                $where[] = [$this->table_name.'.way_name','like','%'.trim($param['way_name']).'%'];
            }
    ...
    public function GetList(){
        ...
        $data = DB::table($this->table_name)
            ->select($this->table_name.'.'.$this->table_pk.' as id',$this->table_name.'.way_name')
            ->where($where)
    ...
    public function Form($jump_type,$id=0)
    {
        ...
        return view('/Admin/House/PaymentWay/Form',$data);
    }
     ...
     * @param $validator
     */
    public function Add(Request $request,RulesPaymentWay $validator)
    {
        ...
        if(!empty($info))
        {
            return $this->common_class->ajaxDataReturnFormat(1,'付款方式已存在，请勿重复添加');
        }
     ...
     * @param $validator
     */
    public function Edit(Request $request,RulesPaymentWay $validator)
    {
     ...
     * @param $validator
     */
    public function Delete(Request $request,RulesPaymentWay $validator)
    {
