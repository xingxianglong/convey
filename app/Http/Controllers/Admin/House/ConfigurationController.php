...
use App\Rules\Admin\House\Configuration as RulesConfiguration;
...
class ConfigurationController extends BaseController
{
    ...
    {
        return view('Admin/House/Configuration/Index');
    }
    ...
    public function GetPage(Request $request)
    {
        ...
        if(count($param) > 0){
            ...
            if(isset($param['configuration_name']) && !empty($param['configuration_name'])){
                $where[] = [$this->table_name.'.configuration_name','like','%'.trim($param['configuration_name']).'%'];
            }
    ...
    public function Form($jump_type,$id=0)
    {
        ...
        return view('/Admin/House/Configuration/Form',$data);
    }
     ...
     * @param $validator
     */
    public function Add(Request $request,RulesConfiguration $validator)
    {
        ...
        $configuration_name = $all['configuration_name']; //配置名称
        $icon = $all['icon']; //图标
        ...
        if(!empty($info))
        {
            return $this->common_class->ajaxDataReturnFormat(1,'配置已存在，请勿重复添加');
        }
     ...
     * @param $validator
     */
    public function Edit(Request $request,RulesConfiguration $validator)
    {
     ...
     * @param $validator
     */
    public function Delete(Request $request,RulesConfiguration $validator)
    {
