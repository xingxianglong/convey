...
use App\Rules\Admin\House\BrandApartment as RulesBrandApartment;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class BrandApartmentController extends BaseController
{
    public $table_name = 'web_house_brand_apartment';
    public $table_pk = 'apartment_id';
    ...
    {
        return view('Admin/House/BrandApartment/Index');
    }
    ...
    public function GetPage(Request $request)
    {
        ...
        if(count($param) > 0){
            ...
            if(isset($param['apartment_name']) && !empty($param['apartment_name'])){
                $where[] = [$this->table_name.'.apartment_name','like','%'.trim($param['apartment_name']).'%'];
            }
    ...
    public function Form($jump_type,$id=0)
    {
        ...
        return view('/Admin/House/BrandApartment/Form',$data);
    }
     ...
     * @param $validator
     */
    public function Add(Request $request,RulesBrandApartment $validator)
    {
        ...
        $apartment_name = $all['apartment_name']; //公寓名称
        ...
        if(!empty($info))
        {
            return $this->common_class->ajaxDataReturnFormat(1,'公寓已存在，请勿重复添加');
        }
     ...
     * @param $validator
     */
    public function Edit(Request $request,RulesBrandApartment $validator)
    {
        ...
        $id = $all['id']; //id
        $apartment_name = $all['apartment_name']; //建筑类型
        $is_show = $all['is_show']; //是否上架
        $is_top = $all['is_top']; //是否置顶
        $sort = $all['sort']; //排序号
     ...
     * @param $validator
     */
    public function Delete(Request $request,RulesBrandApartment $validator)
    {
