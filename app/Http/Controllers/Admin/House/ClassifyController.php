...
use App\Rules\Admin\House\Classify as RulesClassify;
...
class ClassifyController extends BaseController
{
    ...
    {
        return view('Admin/House/Classify/Index');
    }
    ...
    public function GetPage(Request $request)
    {
        ...
        if(count($param) > 0){
            ...
            if(isset($param['classify_name']) && !empty($param['classify_name'])){
                $where[] = [$this->table_name.'.classify_name','like','%'.trim($param['classify_name']).'%'];
            }
    ...
    public function GetList(){
        ...
        $data = DB::table($this->table_name)
            ->select($this->table_name.'.'.$this->table_pk.' as id',$this->table_name.'.classify_name')
            ->where($where)
    ...
    public function Form($jump_type,$id=0)
    {
        ...
        return view('/Admin/House/Classify/Form',$data);
    }
     ...
     * @param $validator
     */
    public function Add(Request $request,RulesClassify $validator)
    {
        ...
        $classify_name = $all['classify_name']; //分类名称
        $note = $all['note']; //备注
        $icon = $all['icon']; //图标
        $icon_ext = $all['icon_ext']; //图标文件后缀名
        $icon_size = $all['icon_size']; //图标文件大小
        ...
        if(!empty($info))
        {
            return $this->common_class->ajaxDataReturnFormat(1,'分类已存在，请勿重复添加');
        }
     ...
     * @param $validator
     */
    public function Edit(Request $request,RulesClassify $validator)
    {
     ...
     * @param $validator
     */
    public function Delete(Request $request,RulesClassify $validator)
    {
