...
use App\Rules\Admin\House\Direction as RulesDirection;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class DirectionController extends BaseController
{
    public $table_name = 'web_house_direction';
    public $table_pk = 'direction_id';
    ...
    {
        return view('Admin/House/Direction/Index');
    }
    ...
    public function GetPage(Request $request)
    {
        ...
        if(count($param) > 0){
            ...
            if(isset($param['direction_name']) && !empty($param['direction_name'])){
                $where[] = [$this->table_name.'.direction_name','like','%'.trim($param['direction_name']).'%'];
            }
    ...
    public function GetList(){
        ...
        $data = DB::table($this->table_name)
            ->select($this->table_name.'.'.$this->table_pk.' as id',$this->table_name.'.direction_name')
            ->where($where)
    ...
    public function Form($jump_type,$id=0)
    {
        ...
        return view('/Admin/House/Direction/Form',$data);
    }
     ...
     * @param $validator
     */
    public function Add(Request $request,RulesDirection $validator)
    {
        ...
        $direction_name = $all['direction_name']; //朝向名称
        ...
        if(!empty($info))
        {
            return $this->common_class->ajaxDataReturnFormat(1,'朝向已存在，请勿重复添加');
        }
     ...
     * @param $validator
     */
    public function Edit(Request $request,RulesDirection $validator)
    {
     ...
     * @param $validator
     */
    public function Delete(Request $request,RulesDirection $validator)
    {
