...
use App\Rules\Admin\House\Decorate as RulesDecorate;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class DecorateController extends BaseController
{
    public $table_name = 'web_house_decorate';
    public $table_pk = 'decorate_id';
    ...
    {
        return view('Admin/House/Decorate/Index');
    }
    ...
    public function GetPage(Request $request)
    {
        ...
        if(count($param) > 0){
            ...
            if(isset($param['decorate_name']) && !empty($param['decorate_name'])){
                $where[] = [$this->table_name.'.decorate_name','like','%'.trim($param['decorate_name']).'%'];
            }
    ...
    public function GetList(){
        ...
        $data = DB::table($this->table_name)
            ->select($this->table_name.'.'.$this->table_pk.' as id',$this->table_name.'.decorate_name')
            ->where($where)
    ...
    public function Form($jump_type,$id=0)
    {
        ...
        return view('/Admin/House/Decorate/Form',$data);
    }
     ...
     * @param $validator
     */
    public function Add(Request $request,RulesDecorate $validator)
    {
        ...
        $decorate_name = $all['decorate_name']; //装修名称
        ...
        if(!empty($info))
        {
            return $this->common_class->ajaxDataReturnFormat(1,'装修已存在，请勿重复添加');
        }
     ...
     * @param $validator
     */
    public function Edit(Request $request,RulesDecorate $validator)
    {
     ...
     * @param $validator
     */
    public function Delete(Request $request,RulesDecorate $validator)
    {
