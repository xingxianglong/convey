...
use App\Rules\Admin\Advertising\Banner as RulesBanner;
...
class BannerController extends BaseController
{
    ...
    {
        return view('Admin/Advertising/Banner/Index');
    }
    ...
    public function GetPage(Request $request)
    {
        ...
        $data = DB::table($this->table_name)
            ->select($this->table_name.'.*',$this->table_name.'.'.$this->table_pk.' as id','update_administrator.administrator_name as update_administrator_name','web_advertising_banner_location.location_name')
            ...
            ->leftJoin('web_advertising_banner_location',$this->table_name.'.location_id','=','web_advertising_banner_location.location_id')
            ->where($where)
    ...
    public function Form($jump_type,$id=0)
    {
        ...
        $location = new BannerLocationController();
        $location_data = $location->GetList();
        $data['location_data'] = $location_data['data'];
        ...
        return view('/Admin/Advertising/Banner/Form',$data);
    }
     ...
     * @param $validator
     */
    public function Add(Request $request,RulesBanner $validator)
    {
        ...
        $jump_type = isset($all['jump_type']) ? $all['jump_type'] : 0; //跳转类型
        $jump_id = isset($all['jump_id']) ? $all['jump_id'] : 0; //跳转id
        $img = $all['img']; //图片
        $img_ext = $all['img_ext']; //图片文件后缀名
        $img_size = $all['img_size']; //图片文件大小
     ...
     * @param $validator
     */
    public function Edit(Request $request,RulesBanner $validator)
    {
     ...
     * @param $validator
     */
    public function Delete(Request $request,RulesBanner $validator)
    {
