...
use App\Rules\Admin\Store\ConsultantPosition as RulesConsultantPosition;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class ConsultantPositionController extends BaseController
{
    public $table_name = 'web_store_consultant_position';
    public $table_pk = 'position_id';
    ...
    {
        return view('Admin/Store/ConsultantPosition/Index');
    }
    ...
    public function GetPage(Request $request)
    {
        ...
        if(count($param) > 0){
            ...
            if(isset($param['position_name']) && !empty($param['position_name'])){
                $where[] = [$this->table_name.'.position_name','like','%'.trim($param['position_name']).'%'];
            }
    ...
    public function Form($jump_type,$id=0)
    {
        ...
        return view('/Admin/Store/ConsultantPosition/Form',$data);
    }
     ...
     * @param $validator
     */
    public function Add(Request $request,RulesConsultantPosition $validator)
    {
        ...
        $position_name = $all['position_name']; //职位名称
        ...
        if(!empty($info))
        {
            return $this->common_class->ajaxDataReturnFormat(1,'职位已存在，请勿重复添加');
        }
     ...
     * @param $validator
     */
    public function Edit(Request $request,RulesConsultantPosition $validator)
    {
     ...
     * @param $validator
     */
    public function Delete(Request $request,RulesConsultantPosition $validator)
    {
